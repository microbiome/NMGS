function [Z_vecs_mat,beta_vecs_array,phi_vecs_array,eta_vecs_array,T_mat_array,I_mat_array,theta_vec_mat] = NicheNeutralGibbs(X_mat,G,N_its)
...
% variables;
% beta_vecs_array = N by G by N_its array of niche probability vectors;
% phi_vecs_array = N by G by S+1 by N_its array of local niche community
% vectors;
...
beta_vecs_array = zeros(N,G,N_its);
gamma_rnd_mat = gamrnd(1,1,N,G);
beta_vecs_array(:,:,1) = gamma_rnd_mat ./ repmat(sum(gamma_rnd_mat,2),1,G);
phi_vecs_array = zeros(N,G,S+1,N_its);
gamma_rnd_array = gamrnd(1,1,N,G,S+1);
phi_vecs_array(:,:,:,1) = gamma_rnd_array ./ repmat(sum(gamma_rnd_array,3),[1 1 S+1]);
...
for n_it = 2:N_its
    ...
    for i = 1:N
        Dirichlet_params = squeeze(sum(n_array(i,:,:))) + gamma;
        gamma_rnd_vec = gamrnd(Dirichlet_params,1)';
        beta_vecs_array(i,:,n_it) = gamma_rnd_vec / sum(gamma_rnd_vec);
    end
    for i = 1:N
        for k = 1:G
            Dirichlet_params2 = (I_mat_array(i,k,n_it-1) * eta_vecs_array(k,:,n_it-1)) + [n_array(i,:,k) 0];
            gamma_rnd_vec2 = gamrnd(Dirichlet_params2,1);
            phi_vecs_array(i,k,:,n_it) = gamma_rnd_vec2 / sum(gamma_rnd_vec2);
        end
    end
    for k = 1:G
