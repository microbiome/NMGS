...
```{r metalocal, message=FALSE, warning=FALSE, fig.align='center', out.width='1\\textwidth'}
boxplot(metacommunity$p, las = 1, xlab = "Species index", ylab = "Abundance", main = "Metacommunity composition")
```
...
full <- qplot(stats$LN, binwidth = 1, geom = "histogram") + geom_vline(x = unique(stats$LO), linetype = 2)
...
local <- qplot(stats$LL, binwidth = 1, geom = "histogram") + geom_vline(x = unique(stats$LO), linetype = 2)
